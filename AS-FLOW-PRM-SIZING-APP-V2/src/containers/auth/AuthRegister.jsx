...
const AuthRegister = (baseRoute) => {
    ...
    const handleSubmit = async (e) => {
        ...
        setLoading(true);
        try {
            // Dispatch the Redux register thunk with registration data
            // This automatically routes to GraphQL or REST based on VITE_API_GATEWAY_ENABLED
            await dispatch(register({
                name: form.Name.trim(),
                email: form.Email.trim().toLowerCase(),
                password: form.Password,
                AppType: baseRoute?.callingProject || 'PRVPA-ONLINE',
            })).unwrap();

            setSuccessMsg('Account created successfully! Redirecting to login…');
            if(baseRoute?.callingProject==="PRVPA-ONLINE"){
                ...
                <Box sx={{
                    ...
                    <form onSubmit={handleSubmit} noValidate>
                        ...
                        <Typography variant="body2" sx={{ textAlign: 'center', color: 'text.secondary' }}>
                            ...
                            <Link to="/" style={{ color: '#00aa7e', fontWeight: 600, textDecoration: 'none' }}>
                                Sign in
                            </Link>
                        </Typography>
                    </form>
