import { BigNumber, checkForValidValue } from "./convertUnit";
...
const parseTokens = (tokens) => {
  ...
  const parseFunction = (token) => {
    ...
    const args = match[2] ? match[2].split(',').map(arg => parseTokens(categorizeTokens(tokenize(arg.trim())))) : [];
...
export const evaluateExpression = (expression, variables) => {
  ...
  const parsedExpression = parseTokens(categorizedTokens);
...
// Example usage
// const expression = "!IsRuptureDisc?'1.0':(RuptureDiscKcFd!=undefined&&RuptureDiscKcFd!=null&&RuptureDiscKcFd!=1.0?RuptureDiscKcFd:'0.900')";
// const variables = {
//   IsRuptureDisc: false,
//   RuptureDiscKcFd: false,
//   myFunction: (a, b) => a + b
// };
